...
# momentum = 0.9 # 在梯度下降过程中解决mini-batch SGD优化算法更新幅度摆动大的问题，使得收敛速度更快
                                                                          ...
                                                                          transform=torchvision.transforms.Compose(
                                                                              [torchvision.transforms.RandomAffine(
                                                                                  ...
                                                                                  torchvision.transforms.RandomRotation(
                                                                                      (-10, 10)),  # 将图片随机旋转（-10,10）度
                                                                                  torchvision.transforms.ToTensor(),
                                                                                  # 将PIL图片或者numpy.ndarray转成Tensor类型
...
# 定义训练函数
def train(network,epoch,device,train_loader, optimizer):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        # loss = F.CrossEntropyLoss(output, target.to(device))
...
def test(network,epoch,device,test_loader,optimizer):
    ...
    test_acc = correct / len(test_loader.dataset)  # 平均测试准确率
    test_acces.append(test_acc.cpu().numpy().tolist())
    ...
    print('\r Test set \033[1;31m{}\033[0m : Avg. loss: {:.4f}, Accuracy: {}/{}  \033[1;31m({:.2f}%)\033[0m\n' \
          .format(epoch, test_loss, correct, len(test_loader.dataset), 100. * test_acc), end='')
    return test_acc
...
def main():
    ...
    test(network, 1, device, test_loader,optimizer)
    ### 训练！！！ 并在每个epoch之后测试 ###
    ###################################################
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        train(network, epoch, device, train_loader, optimizer)
        test(network, epoch, device, test_loader,optimizer)

        if not os.path.isdir('curve'):
            os.mkdir('curve')
        torch.save({'train_acc': train_acces, 'test_acc': test_acces}, os.path.join('curve', 'RMSprop_acc'))
        torch.save({'train_loss': train_losses, 'test_loss': test_losses}, os.path.join('curve', 'RMSprop_loss'))
